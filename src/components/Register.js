...
const Register = () => {
    ...
    const registerUM = gql`
        mutation registerU($name: String!, $email: String!, $password: String!, $confirmPassword: String!, $tel: String!, $city: String!, $country: String!, $age: Int!) {
            registerU(name: $name, email: $email, password: $password, confirmPassword: $confirmPassword, tel: $tel, city: $city, country: $country, age: $age) {
                name
                email
                password
                confirmPassword
    ...
    const registerBM = gql`
        mutation registerB($name: String!, $email: String!, $password: String!, $confirmPassword: String!, $tel: String!, $city: String!, $country: String!, $age: Int!) {
            registerB(name: $name, email: $email, password: $password, confirmPassword: $confirmPassword, tel: $tel, city: $city, country: $country, age: $age) {
                name
                email
                password
                confirmPassword
    ...
    const [registerU] = useMutation(registerUM, {
        optimisticResponse: true,
        update(proxy, result) {
            if (result.data.registerU !== undefined) {
                console.log(result.data.registerU)
                document.cookie = `user=${JSON.stringify(result.data.registerU)}`
                setLoc('/')
    ...
    const [registerB] = useMutation(registerBM, {
        optimisticResponse: true,
        update(proxy, result) {
            if (result.data.registerB !== undefined) {
                console.log(result.data.registerB)
                document.cookie = `biker=${JSON.stringify(result.data.registerB)}`
                setLoc('/')
                window.location.reload()
            }
        }
    ...
    return (
        <div className="con">
            ...
            <Checkbox value={flag} onChange={e => setFlag(e.target.checked)}></Checkbox>
